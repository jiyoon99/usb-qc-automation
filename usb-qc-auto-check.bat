...
:: ============================================================================
::  USB AUTO CHECK - QC TERMINAL EDITION
::  기능 유지 + 터미널 UI 정리 + 출고 BIOS 진입 옵션 유지
:: ============================================================================

title QC AUTOMATION :: QC TERMINAL
color 0A
...
set "QC_LOGO_FILE=%TEMP%\halfet_qc_logo.txt"
...
if /I "%~1"=="--preview-final" (
    ...
    set "C_EXTEND_RESULT=이미 확장되어 있음"
...
call :boxTop
call :boxTitle "QC // LOCAL NODE OPS :: LIVE QC TRACE"
call :boxSep
...
set "CURRENT_DETAIL=Preparing local QC console..."
...
:tools
title [QC ACTIVE] QC USB AUTO CHECK
...
:: C 드라이브 자동 확장
set "C_EXTEND_RESULT=확인 중"
call :section "C DRIVE EXPANSION" "PARTITION ANALYSIS"
powershell -NoProfile -Command "$ErrorActionPreference='Stop'; try { $partition = Get-Partition -DriveLetter 'C'; $size = Get-PartitionSupportedSize -DriveLetter 'C'; $currentGB = [math]::Round($partition.Size / 1GB, 2); $maxGB = [math]::Round($size.SizeMax / 1GB, 2); Write-Host ('  현재 크기: ' + $currentGB + ' GB'); Write-Host ('  최대 확장 가능: ' + $maxGB + ' GB') } catch { Write-Host '  현재 크기: 확인 불가'; Write-Host '  최대 확장 가능: 확인 불가' }" >nul 2>&1
timeout /t 2 >nul

for /f "usebackq delims=" %%C in (`powershell -NoProfile -Command "$ErrorActionPreference='Stop'; try { $partition = Get-Partition -DriveLetter 'C'; $size = Get-PartitionSupportedSize -DriveLetter 'C'; if (($size.SizeMax - $partition.Size) -le 1MB) { 'ALREADY' } else { Resize-Partition -DriveLetter 'C' -Size $size.SizeMax | Out-Null; 'EXTENDED' } } catch { 'FAILED' }"`) do set "C_EXTEND_CODE=%%C"

if "%C_EXTEND_CODE%"=="EXTENDED" (
    set "C_EXTEND_RESULT=확장 완료"
)
if "%C_EXTEND_CODE%"=="ALREADY" (
    set "C_EXTEND_RESULT=이미 확장되어 있음"
)
if not "%C_EXTEND_CODE%"=="EXTENDED" if not "%C_EXTEND_CODE%"=="ALREADY" (
    set "C_EXTEND_RESULT=확장 실패"
    set "OVERALL_RESULT=확인 필요"
)
set "CURRENT_STAGE=C DRIVE EXPANSION"
set "CURRENT_DETAIL=C drive partition status updated"
set "LAST_ACTION=C drive result code: !C_EXTEND_CODE!"
call :renderDashboard
timeout /t 2 >nul
...
title QC AUTOMATION :: FINAL REPORT
color 0A
mode con: cols=136 lines=38 >nul 2>&1
call :centerConsole
set "CURRENT_STAGE=FINAL REPORT"
...
:prepareLogo
powershell -NoProfile -ExecutionPolicy Bypass -Command "$w=132;$tag='[portfolio | 1.0v]';$on=([string][char]9608)+([string][char]9608);$sp='  ';$H=@('1001','1001','1111','1001','1001');$A=@('1110','1001','1111','1001','1001');$L=@('1000','1000','1000','1000','1111');$F=@('1111','1000','1110','1000','1000');$E=@('1111','1000','1110','1000','1111');$T=@('11111','00100','00100','00100','00100');$sets=@($H,$A,$L,$F,$E,$T);$rows=@();for($i=0;$i -lt 5;$i++){$r='';foreach($set in $sets){if($r.Length -gt 0){$r+='00'};$r+=$set[$i]};$rows+=$r};$out=@();foreach($r in $rows){$line='';for($c=0;$c -lt $r.Length;$c++){if($r[$c] -eq '1'){$line+=$on}else{$line+=$sp}};$out+=$line.TrimEnd()};$max=($out|Measure-Object -Property Length -Maximum).Maximum;$left=[Math]::Max(0,[int](($w-$max)/2));$lines=foreach($l in $out){(' '*$left)+$l};$pos=[Math]::Max(0,$w-$tag.Length);if($lines[0].Length -lt $pos){$lines[0]=$lines[0].PadRight($pos)+$tag}else{$lines[0]=$lines[0]+'  '+$tag};[IO.File]::WriteAllLines($env:QC_LOGO_FILE,$lines,[Text.UTF8Encoding]::new($false))" >nul 2>&1
exit /b
